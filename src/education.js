...
function _createNoteStaff(pitch) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    line.setAttribute('y1', String(30 + i * 15));
    line.setAttribute('x2', '280');
    line.setAttribute('y2', String(30 + i * 15));
  ...
  if (notePos.ledgerLines > 0) {
    for (let i = 0; i < notePos.ledgerLines; i++) {
      const ledger = document.createElementNS('http://www.w3.org/2000/svg', 'line');
      ledger.setAttribute('x1', '140');
      ledger.setAttribute('y1', String(105 + i * 15));
      ledger.setAttribute('x2', '160');
      ledger.setAttribute('y2', String(105 + i * 15));
      ledger.setAttribute('stroke', '#4a5568');
      ledger.setAttribute('stroke-width', '1.5');
      svg.appendChild(ledger);
    }
  } else if (notePos.ledgerLines < 0) {
    for (let i = 0; i < Math.abs(notePos.ledgerLines); i++) {
      const ledger = document.createElementNS('http://www.w3.org/2000/svg', 'line');
      ledger.setAttribute('x1', '140');
      ledger.setAttribute('y1', String(15 - (i + 1) * 15));
      ledger.setAttribute('x2', '160');
      ledger.setAttribute('y2', String(15 - (i + 1) * 15));
      ledger.setAttribute('stroke', '#4a5568');
      ledger.setAttribute('stroke-width', '1.5');
      svg.appendChild(ledger);
    }
  ...
  stem.setAttribute('y2', notePos.stemUp ? String(notePos.y - 45) : String(notePos.y + 45));
...
function _pitchToStaffPosition(pitch) {
  // Treble clef: E4 is on the first line (bottom line)
  ...
  // Map pitch to staff position (0 = bottom line E4, increases upward)
  ...
  // Calculate position relative to E4 (first line)
  const e4Pitch = 64; // E4 MIDI
  const octavesFromE4 = oct - 4;
  const positionFromE4 = octavesFromE4 * 7 + baseDiatonic - 2; // -2 because E is at position 2 in diatonicMap
  
  // Staff Y coordinates: bottom line (E4) = 105, each step up = -7.5
  const y = 105 - (positionFromE4 * 7.5);
  
  // Determine stem direction (stems down if note is above middle line)
  const stemUp = positionFromE4 < 4; // Middle line is B4 (position 4)
  
  // Determine ledger lines
  let ledgerLines = 0;
  if (positionFromE4 < 0) {
    ledgerLines = Math.ceil(Math.abs(positionFromE4) / 2);
  } else if (positionFromE4 > 8) {
    ledgerLines = Math.ceil((positionFromE4 - 8) / 2);
  }
  ...
  return { y, stemUp, ledgerLines, accidental };
}
...
function _generateNoteOptions(correctPitch) {
  ...
  // Add octave variations if needed
  if (options.length < 4) {
