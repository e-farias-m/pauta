...
function parseFing(fingArr, altIndex) {
  ...
  // Recorder format: characters are hole numbers ('0'-'9' = covered, 'X' = pinched thumb)
  // Woodwind/brass format: characters are states ('0'=open, '1'=covered, '2'=half-hole)
  const isRecorderFormat = /[3-9X]/.test(fing);
  if (isRecorderFormat) {
...
function renderLargeRecorder(fingArr, altIndex = 0, noteName = '') {
  ...
  const SVG_HOLES = [
    [ '0', 24.8, -295.0, 6.3, false ],   // thumb (centered on body)
  ...
  const THUMB_INDICATOR = [2, -295.0, 6.3];

  // Built-in hole base shapes (inner-space coordinates) — main holes 0-5
  const holeDefs = [
    [24.8,-295.0,6.3],[24.8,-312.5,6.3],[24.8,-350.0,6.3],
