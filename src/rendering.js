...
function renderWoodwindDiagram(fingArr, type, altIndex = 0, noteName = '', regActive = false) {
  ...
  if (type === 'clarinet') {
    ...
    rhLbl.setAttribute('x', String(CX + 12)); rhLbl.setAttribute('y', String(sepY + 14));
  ...
  if (type === 'flute') {
    ...
    tube.setAttribute('x1', '35'); tube.setAttribute('y1', String(CY));
    tube.setAttribute('x2', '268'); tube.setAttribute('y2', String(CY));
    tube.setAttribute('stroke', '#888'); tube.setAttribute('stroke-width', '8');
    tube.setAttribute('stroke-linecap', 'round');
    svg.appendChild(tube);
    const emb = document.createElementNS(ns, 'ellipse');
    emb.setAttribute('cx', '30'); emb.setAttribute('cy', String(CY));
    emb.setAttribute('rx', '5'); emb.setAttribute('ry', '3');
    emb.setAttribute('fill', 'none'); emb.setAttribute('stroke', '#999');
    emb.setAttribute('stroke-width', '1.2');
    svg.appendChild(emb);
    function drawHole(cx, cy, r, isCovered, shape) {
      const el = shape === 'rect'
    ...
    [[2,'L1',72],[3,'L2',106],[4,'L3',140],[6,'R1',180],[7,'R2',214],[8,'R3',248]].forEach(([idx,label,cx]) => {
      drawHole(cx, CY, R, pressed.has(idx));
      putLabel(cx + R + 4, CY + 3, label);
    });
    drawHole(60, 90, 4.5, pressed.has(0));
    putLabel(60 + 5, 90 + 3, 'Th');
    drawHole(88, 90, 0, pressed.has(1), 'rect');
    putLabel(88 + 5, 90 + 3, 'B♭');
    drawHole(158, 44, 0, pressed.has(5), 'rect');
    putLabel(158 + 5, 44 + 3, 'G♯');
    drawHole(255, 90, 0, pressed.has(9), 'rect');
    putLabel(255 + 5, 90 + 3, 'E♭');
    drawHole(218, 44, 3.5, pressed.has(10));
    putLabel(218 + 4, 44 + 3, 'C♯');
    drawHole(274, CY, 6, pressed.has(11));
    putLabel(274 + 7, CY + 3, 'C');
  ...
  if (type === 'sax' || type === 'tenor-sax') {
    ...
    bell.setAttribute('d', 'M 78,161 L 62,198 L 114,198 L 96,161 Z');
    ...
    octRing.setAttribute('cx', '128'); octRing.setAttribute('cy', '22'); octRing.setAttribute('r', '4');
    ...
    if (regActive) diagHighlight(svg, 128, 22, 4, APP.playing);
    const regLbl = document.createElementNS(ns, 'text');
    regLbl.setAttribute('x', '128'); regLbl.setAttribute('y', '18');
    ...
    drawHole(96, 34, 0, pressed.has(12), 'rect');
    putLabel(96 + 6, 34 + 3, 'F');
    drawHole(101, 48, 0, pressed.has(11), 'rect');
    putLabel(101 + 6, 48 + 3, 'E♭');
    drawHole(106, 62, 0, pressed.has(10), 'rect');
    putLabel(106 + 6, 62 + 3, 'D');
    [[0,'L1',96,68],[1,'L2',90,90],[2,'L3',84,114]].forEach(([idx,label,x,y]) => {
      drawHole(x, y, R, pressed.has(idx));
      putLabel(x + R + 6, y + 3, label);
    });
    drawHole(66, 86, 0, pressed.has(3), 'rect');
    putLabel(66 + 6, 86 + 3, 'G♯');
    const sepY = 130;
    const sep = document.createElementNS(ns, 'line');
    sep.setAttribute('x1', '62'); sep.setAttribute('y1', String(sepY));
    sep.setAttribute('x2', '112'); sep.setAttribute('y2', String(sepY));
    sep.setAttribute('stroke', '#999'); sep.setAttribute('stroke-width', '1');
    svg.appendChild(sep);
    [[4,'R1',95,140],[5,'R2',88,162],[6,'R3',81,184]].forEach(([idx,label,x,y]) => {
      drawHole(x, y, R, pressed.has(idx));
      putLabel(x + R + 6, y + 3, label);
    });
    drawHole(132, 142, 0, pressed.has(8), 'rect');
    putLabel(132 + 6, 142 + 3, 'B♭');
    drawHole(132, 166, 0, pressed.has(9), 'rect');
    putLabel(132 + 6, 166 + 3, 'C');
    drawHole(118, 152, 4, pressed.has(7));
    putLabel(118 + 5, 152 + 3, 'C');
    drawHole(82, 194, 5, pressed.has(13));
    putLabel(82 + 6, 194 + 3, 'B♭');
    drawHole(99, 194, 5, pressed.has(14));
    putLabel(99 + 6, 194 + 3, 'B');
    const lhLbl = document.createElementNS(ns, 'text');
    lhLbl.setAttribute('x', '92'); lhLbl.setAttribute('y', '52');
    lhLbl.setAttribute('style', 'font-family:var(--pauta-font-sans);font-size:6px;font-weight:700;fill:#999;text-anchor:middle;pointer-events:none');
    lhLbl.textContent = 'LH';
    svg.appendChild(lhLbl);
    const rhLbl = document.createElementNS(ns, 'text');
    rhLbl.setAttribute('x', '92'); rhLbl.setAttribute('y', String(sepY + 14));
    rhLbl.setAttribute('style', 'font-family:var(--pauta-font-sans);font-size:6px;font-weight:700;fill:#999;text-anchor:middle;pointer-events:none');
    rhLbl.textContent = 'RH';
    svg.appendChild(rhLbl);
    diagNoteName(svg, noteName, w / 2, 14);
  ...
  if (type === 'oboe') {
    ...
    svg.appendChild(lhLbl);
    const rhLbl = document.createElementNS(ns, 'text');
    rhLbl.setAttribute('x', String(CX + 9)); rhLbl.setAttribute('y', String(sepY + 14));
    rhLbl.setAttribute('style', 'font-family:var(--pauta-font-sans);font-size:6px;font-weight:700;fill:#999;text-anchor:middle;pointer-events:none');
    rhLbl.textContent = 'RH';
    svg.appendChild(rhLbl);
    diagNoteName(svg, noteName, w / 2, 12);
