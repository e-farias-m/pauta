...
function _renderNewScoreDialog(restoreScroll) {
  ...
  // Family tab buttons
  const famBtns = families.map(f => pillBtn(f, {active: f === _ndFamily, action: 'selectNDFamily', className: `data-family="${f}"`})).join('');
...
function pillBtn(label, {active = false, action = '', className = ''} = {}) {
  const actionAttr = action ? ` data-action="${action}"` : '';
  const classAttr = className ? ` ${className}` : '';
  const activeClass = active ? ' active' : '';
  return `<button class="pauta-pill${activeClass}${classAttr}"${actionAttr}>${label}</button>`;
}
