...
function showScoreMenu(btn) {
  ...
  // Show assignment items if assignments exist in the score
  const hasAssignments = (APP.score?.assignments?.length || 0) > 0;
  if (hasAssignments || APP.assignmentMode) {
    items.splice(5, 0, {sep:true}, {label:'📝 Assignment', fn:showAssignmentSubmenu});
  } else {
    items.splice(5, 0, {sep:true}, {label:'📝 Create Assignment', fn:showAssignmentDialog});
  }
...
function showFileMenu(btn) {
  showDropdown(btn, [
    ...
    {sep:true},
    {label:'🖨️ Print / Save as PDF', fn:printScore},
...
function showNewScoreDialog() {
  ...
  // Default level from current kit (if any), or advanced
...
function _renderNewScoreDialog() {
  ...
  makeModal(`
    ...
    <div style="background:rgba(192,86,33,0.04);border:1px solid rgba(192,86,33,0.12);border-radius:10px;padding:8px 6px;margin-top:6px">
      <label style="color:rgba(74,85,104,0.70);font-size:10px;text-transform:uppercase;letter-spacing:0.3px;display:block;text-align:center;margin-bottom:4px;font-weight:600">Key &amp; Time Signature</label>
    ...
    ${FAMILY_KIT_MAP[_ndFamily] ? (() => {
      ...
      return `
    <div style="background:rgba(192,86,33,0.04);border:1px solid rgba(192,86,33,0.12);border-radius:10px;padding:8px 6px;margin-top:6px">
      <label style="color:rgba(74,85,104,0.70);font-size:10px;text-transform:uppercase;letter-spacing:0.3px;display:block;text-align:center;margin-bottom:4px;font-weight:600">Level<span style="font-weight:400;color:rgba(74,85,104,0.5)"> — ${_kitName}</span></label>
    ...
    <label style="color:rgba(74,85,104,0.70);font-size:10px;text-transform:uppercase;
                  letter-spacing:0.3px;margin-top:4px;display:block;font-weight:600">Instruments <span id="nd-instr-count" style="font-weight:400">(0)</span></label>
...
function createNewScore() {
  ...
  // Detect kit from selected instruments and apply
...
async function loadScript(src) {
  ...
  if ('caches' in window) {
    try {
      const cache    = await caches.open(_LIB_CACHE);
